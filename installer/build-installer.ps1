$ErrorActionPreference = 'Stop'

$env:Path += ';C:\msys64\mingw64\bin'
$env:Path += ';C:\Program Files\Inno Setup 7'
$env:Path += ';C:\Program Files (x86)\Inno Setup 6'

$ProjectDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$OutDir = Join-Path $ProjectDir 'dist'
$AppDir = Join-Path $OutDir 'app'
$InstallerScript = Join-Path $ProjectDir 'luc-installer.iss'
$BuildExe = Join-Path $ProjectDir 'luc.exe'
$MingwBin = 'C:\msys64\mingw64\bin'

if (Test-Path $AppDir) {
    Remove-Item -Recurse -Force $AppDir
}
New-Item -ItemType Directory -Force -Path $AppDir | Out-Null

$gcc = Get-Command gcc -ErrorAction SilentlyContinue
if (-not $gcc) {
    throw 'MinGW gcc was not found in PATH. Open MSYS2 MinGW64 shell or add mingw64\bin to PATH.'
}

& gcc -O2 -s -std=c99 -DLUC_WINDOW -DLUC_NO_TTF -DLUC_NO_IMAGE -mwindows -o $BuildExe (Join-Path $ProjectDir 'main.c') -lm -lSDL2
if ($LASTEXITCODE -ne 0) {
    throw 'Build failed. Check SDL2 headers/libs and MinGW installation.'
}

Copy-Item $BuildExe $AppDir

$DllNames = @(
    'SDL2.dll'
)

foreach ($dll in $DllNames) {
    $src = Join-Path $MingwBin $dll
    if (Test-Path $src) {
        Copy-Item $src $AppDir
    }
}

$iscc = Get-Command iscc -ErrorAction SilentlyContinue
if (-not $iscc) {
    $isccCandidates = @(
        'C:\Program Files\Inno Setup 7\ISCC.exe',
        'C:\Program Files (x86)\Inno Setup 7\ISCC.exe',
        'C:\Program Files\Inno Setup 6\ISCC.exe',
        'C:\Program Files (x86)\Inno Setup 6\ISCC.exe'
    )
    $foundIscc = $isccCandidates | Where-Object { Test-Path $_ } | Select-Object -First 1
    if (-not $foundIscc) {
        throw 'Inno Setup (ISCC.exe) was not found.'
    }
    $iscc = @{ Source = $foundIscc }
}

& $iscc.Source $InstallerScript
if ($LASTEXITCODE -ne 0) {
    throw 'Installer build failed.'
}

Write-Host 'Installer created successfully in dist\.'
